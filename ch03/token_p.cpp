// 리스트 3-1  어휘 분석 프로그램  token_p.cpp
...
Token nextTkn()                 /* 토큰 추출, 한자나 이스케이프 문자는 비대응 */
{
  TknKind kd;
  int ch0, num = 0;
...
bool is_ope2(int c1, int c2)                      /* 2 문자 연산자라면 참 */
{
  char s[] = "   ";
...
TknKind get_kind(const string& s)
{
  ...
  if (ctyp[s[1]] == Digit) return IntNum;
  return Others;
}
