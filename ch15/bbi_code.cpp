...
public:
  ...
  double pop() {			     /* 읽기 & 삭제(원래 pop과는 다르니 주의) */
...
void syntaxChk()                             /* 구문 검사 */
{
  ...
  for (Pc=1; Pc<(int)intercode.size(); Pc++) {
    ...
    case Gvar: case Lvar:                                   /* 대입문 */
      ...
      (void)get_expression('=', EofLine);                   /* 우변 식 값 */
      break;
    case Return:
      code = nextCode();
      if (code.kind!='?' && code.kind!=EofLine) (void)get_expression(); /* 반 환 값 */
...
void statement()    /* 문 */
{
  ...
  if (code.kind == If ) end_line = endline_of_If(Pc);  /* if 문일 때의 끝 */
  ...
  case While:
    ...
    Pc = end_line + 1;                                 /*              */
    break;
  case For:                                            /* for 제어 변수, 초기값, 최종 값, 증분식 */
    save = nextCode();
    varAdrs = get_memAdrs(save);                       /* 제어 변수 주소 구하기 */

    expression('=', 0);                                /* 초기값 */
    set_dtTyp(save, DBL_T);                            /* 형 확정 */
    Dmem.set(varAdrs, stk.pop());                      /* 초기값을 설정  */
...
int opOrder(TknKind kd)    /* 이항 연산자 우선순위 */
{
  ...
  case Plus:  case Minus:      return 5;    /* _ -       */
...
void fncCall_syntax(int fncNbr)                        /* 함수 호출 검사 */
{
  ...
  if (argCt != Gtable[fncNbr].args)                    /* 인수 개수 검사 */
    err_exit(Gtable[fncNbr].name, "함수의 인수 개수가 잘못 되었습니다");
...
void fncCall(int fncNbr)    /* 함수 호출 */
{
  ...
  // 인수 저장 순서 변경
  for (n=0; n<argCt; n++) vc.push_back(stk.pop());     /* 뒤에서부터 인수 저장 */
...
TknKind lookCode(int line)                             /* line 행의 시작 코드 */
{
...
{
  if (cd.kind != kind2) {
    if (kind2   == EofLine) err_exit("잘못된 기술이빈다: ", kind_to_s(cd));
