...
// 맨 앞에만 출현하는 코드를 초리, 나머지 부분은 convert_rest()로 처리
void convert()
{
  switch (token.kind) {
...
void convert_rest()                          /* 문의 나머지 처리 */
{
  ...
  for (;;) {
    ...
    switch (token.kind) {                    // ↓이 키워드가 도중에 나타나는일은 없다. */
	...
	tblNbr = enter(tmpTb, varId);                   /* 자동 변수 등록 */
      ...
      continue;
    case IntNum: case DblNum:                /* 정수도 double 형으로 저장 */
...
{
  setCode(Option);                           /* 이 행은 비실행이므로 코드 변환은 Option 만 */
...
void fncDecl()    /* 함수 정의 */
{
  ...
  patch_line = setCode(Func, NO_FIX_ADRS);   /* 나중에 end 행 번호를 넣는다 */
  token = nextTkn();

  fncTblNbr = searchName(token.text, 'F');   /* 함수명은 맨 처음에 등록한다 */
  Gtable[fncTblNbr].dtTyp = DBL_T;           /* 함수형은 double로 고정 */
...
void backPatch(int line, int n)              /* line 행에 n을 설정 */
{
...
int setCode(int cd, int nbr)                 /* 코드와 short 값 저장 */
{
  *codebuf_p++ = (char)cd;
...
void push_intercode()                        /* 변환한 내부 코드를 저장 */
{
  ...
  *codebuf_p++ = '\0';
  ...
  codebuf_p = codebuf;                       /* 다음 처리를 대비해 저장할 곳 맨 앞으로 위치 지정 */
}
